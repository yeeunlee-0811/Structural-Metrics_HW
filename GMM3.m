function [gmmparam_init,gmmparam,optW_init,optW_new] = GMM3(gmmparam0,eps,data)
% GMM calculates gmm estimates and standard errors
n = size(data,1);

gi_init = gi3(gmmparam0,eps,data)';

Ainit = (1/n).*cov(gi_init);

optW_init = inv(Ainit);

gmmfnval0 = @(coeff)GMM_beta3(coeff, optW_init, eps,data);
...
gi_new = gi3(gmmparam_init,eps,data)';

Anew = (1/n).*cov(gi_new);

optW_new = inv(Anew);

gmmfnval1 = @(coeff)GMM_beta3(coeff, optW_new, eps, data);
